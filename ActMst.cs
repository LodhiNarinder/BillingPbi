...
{
    public partial class ActMst : Form
    {
        ...
        private void fncLoadGridData()
        {
            try
            {
                ...
                dataGridView1.DataSource = clsConsql.getDatasetsql("SELECT  [ID],[ActCode],[ActNAME],[ActType],[ActAddress] FROM [ActMst] where [ActType]='A' order by [ActCode] desc");
        ...
        {
            DataSet NewCode = clsConsql.getDatasetsql("select RIGHT(CONCAT('000',max([ActCode])+1), 4) from [ActMst] where [ActType]='A' ");
        ...
        private void CreateCycleData()
        {
            ...
            try
            {
                if (txtId.Text.Trim() == "")
                {
                    ...
                    strSqlCr = strSqlCr + " ([ActCode],[ActNAME],[ActType],[ActAddress]) ";
                    strSqlCr = strSqlCr + " values('" + (txtCustCode.Text) + "','" + (txtCustName.Text) + "','" + (txtActType.Text) + "','" + (txtAddress.Text) + "')";
                }
                else
                {
